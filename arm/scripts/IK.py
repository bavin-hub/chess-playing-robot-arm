...
class PickAndPlaceSequence:
    def __init__(self):
        ...
        self.to_arduino_pub = rospy.Publisher("/coords_to_arduino",
                                             Floats,
                                             queue_size=10)
        
        self.sub = rospy.Subscriber("/robot_frame_coords",
                               Float64MultiArray,
                               self.coordinates_callback)


        self.ik = IK_4dof(12.0, 14, 15.1, 6)
        print("started pnp node...")
    ...
    def coordinates_callback(self,msg):
        ...
        pick_theta1, pick_theta2, pick_theta3, pick_theta4 = self.ik((pick_x, pick_y, pick_z))
        pick_clearance_theta1, pick_clearance_theta2, pick_clearance_theta3, pick_clearance_theta4 = self.ik((pick_x, pick_y, pick_z + clearance))
        place_theta1, place_theta2, place_theta3, place_theta4 = self.ik((place_x, place_y, place_z))
        place_clearance_theta1, place_clearance_theta2, place_clearance_theta3, place_clearance_theta4 = self.ik((place_x, place_y, place_z+clearance))
        ...
        self.to_arduino_pub.publish(pnp_sequence)
        print("pnp seq : ", pnp_sequence.data)
